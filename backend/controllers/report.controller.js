...
exports.getDashboardStats = async (req, res, next) => {
  try {
    ...
    const [
      ...
      Visit.aggregate([
        { $match: { clinicId, visitDate: { $gte: todayStart, $lte: todayEnd }, paymentStatus: 'paid' } },
...
exports.getTopDiagnoses = async (req, res, next) => {
  try {
    const { limit = 10 } = req.query;
    const result = await Prescription.aggregate([
      { $match: { clinicId: req.clinicId, diagnosis: { $exists: true, $ne: '' } } },
